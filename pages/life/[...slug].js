...
export async function getStaticPaths({ locales, defaultLocale }) {
  const paths = locales
    .map((locale) => {
      const posts = getBlogPosts('life', locale === defaultLocale ? 'zh-TW' : locale);
...
export async function getStaticProps({ locales, locale, params }) {
  const props = await getPostSlugProps({ category: 'life', locales, locale, params });
...
export default function LifeSlug({ post, authorDetails, prev, next, availableLocales }) {
