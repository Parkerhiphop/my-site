...
export async function getFileBySlug(category, slug, currentLocale = 'zh-TW') {
  const [mdxPath, mdPath] =
    currentLocale === 'zh-TW'
      ? [
          path.join(root, 'data', category, `${slug}/zh-TW.mdx`),
          path.join(root, 'data', category, `${slug}/zh-TW.md`),
        ]
      : [
          path.join(root, 'data', category, `${slug}/${currentLocale}.mdx`),
          path.join(root, 'data', category, `${slug}/${currentLocale}.md`),
        ];
  ...
  relevantFiles.forEach((file) => {
    ...
    if (frontmatter.draft !== true) {
      allFrontMatter.push({
        ...frontmatter,
        category: folder,
        slug: formatSlug(fileName.split('/')[0]),
        date: frontmatter.date ? new Date(frontmatter.date).toISOString() : null,
      });
    }
  });
