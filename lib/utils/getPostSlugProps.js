...
export async function getPostSlugProps({ category, locales, locale, params }) {
  ...
  const aboutMe = await getFileBySlug('about-me', '', locale);
  const authorDetails = [aboutMe.frontMatter];

  // Check available locales for SEO
  const availableLocales = [];
  /* eslint-disable no-await-in-loop */
  for (const ilocal of locales) {
    const iAllPosts = await getAllFilesFrontMatter(category, ilocal);
    const hasPost = iAllPosts.some(
      (ipost) => ipost.slug === post.frontMatter.slug && ipost.slug !== ''
    );
    if (hasPost) availableLocales.push(ilocal);
  }
